...
static const std::u16string DESCRIPTOR = u"ohos.edm.IEnterpriseDeviceMgr";
...
napi_value BrowserAddon::SetManagedBrowserPolicy(napi_env env, napi_callback_info info)
{
    ...
    ASSERT_AND_THROW_PARAM_ERROR(env, MatchValueType(env, argv[ARR_INDEX_ONE], napi_string),
        "bundleName type error");
    ASSERT_AND_THROW_PARAM_ERROR(env, MatchValueType(env, argv[ARR_INDEX_TWO], napi_string),
        "policyName type error");
    ASSERT_AND_THROW_PARAM_ERROR(env, MatchValueType(env, argv[ARR_INDEX_THREE], napi_string),
        "policyValue type error");
    ...
    ASSERT_AND_THROW_PARAM_ERROR(env, ParseString(env, policyName, argv[ARR_INDEX_TWO]),
        "Parameter policyName error");
    ASSERT_AND_THROW_PARAM_ERROR(env, ParseString(env, policyValue, argv[ARR_INDEX_THREE]),
        "Parameter policyValue error");

    if (!policyValue.empty()) {
        cJSON* root = cJSON_Parse(policyValue.c_str());
        ASSERT_AND_THROW_PARAM_ERROR(env, root != nullptr, "policyValue is not a json value");
        cJSON_Delete(root);
    }

    MessageParcel parcelData;
    parcelData.WriteInterfaceToken(DESCRIPTOR);
    parcelData.WriteInt32(WITHOUT_USERID);
    parcelData.WriteParcelable(&elementName);
    parcelData.WriteString(WITHOUT_PERMISSION_TAG);
    parcelData.WriteString(bundleName);
    parcelData.WriteString(policyName);
    parcelData.WriteString(policyValue);
    int32_t ret = BrowserProxy::GetBrowserProxy()->SetManagedBrowserPolicy(parcelData);
    if (FAILED(ret)) {
...
napi_value BrowserAddon::GetManagedBrowserPolicy(napi_env env, napi_callback_info info)
{
    ...
    parcelData.WriteParcelable(&elementName);
    parcelData.WriteString(EdmConstants::Browser::GET_MANAGED_BROWSER_FILE_DATA);
    parcelData.WriteString(bundleName);

    int32_t size = -1;
    void* retData = nullptr;
    int32_t ret = BrowserProxy::GetBrowserProxy()->GetManagedBrowserPolicy(parcelData, &retData, size);
    if (FAILED(ret)) {
        napi_throw(env, CreateError(env, ret));
        return nullptr;
    }
    return CreateArrayBuffer(env, retData, size);
}
