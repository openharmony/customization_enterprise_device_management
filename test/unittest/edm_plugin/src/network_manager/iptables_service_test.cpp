...
#include "iptables_service_test.h"
...
void IptablesSereviceTest::SetUp()
{
    ruleExecuterMock = std::make_shared<RuleExecuterMock>();
    RuleExecuter::instance_ = ruleExecuterMock;

    iptablesService = IptablesService::GetInstance();
    EXPECT_CALL(*ruleExecuterMock, Execute).WillRepeatedly(DoAll(Invoke(PrintExecRule), Return(ERR_OK)));
    iptablesService->Init();
}

void IptablesSereviceTest::TearDown()
{
    RuleExecuter::instance_ = nullptr;
    DaoFactory::instance_ = nullptr;
    IptablesService::instance_ = nullptr;
}

HWTEST_F(IptablesSereviceTest, TestInit, TestSize.Level1)
{
    EXPECT_CALL(*ruleExecuterMock, Execute).WillRepeatedly(DoAll(Invoke(PrintExecRule), Return(ERR_OK)));
    iptablesService->Init();
    EXPECT_EQ(iptablesService->HasInit(), true);
}

HWTEST_F(IptablesSereviceTest, TestAddFilewallSuccess, TestSize.Level1)
{
    EXPECT_CALL(*ruleExecuterMock, Execute).WillRepeatedly(DoAll(Invoke(PrintExecRule), Return(ERR_OK)));
    ...
    for (const auto &item : validRules) {
        ...
        ErrCode ret = iptablesService->AddFirewallRule(validFirewallRule);
        EXPECT_EQ(ret, ERR_OK);
    }
}

HWTEST_F(IptablesSereviceTest, TestAddFilewallFail, TestSize.Level1)
{
    EXPECT_CALL(*ruleExecuterMock, Execute).WillRepeatedly(DoAll(Invoke(PrintExecRule), Return(ERR_OK)));
    ...
    for (const auto &item : invalidRules) {
        ...
        ErrCode ret = iptablesService->RemoveFirewallRule(invalidFirewallRule);
        EXPECT_EQ(ret, EdmReturnErrCode::PARAM_ERROR);
    }
}

HWTEST_F(IptablesSereviceTest, TestAddFilewallError, TestSize.Level1)
{
    EXPECT_CALL(*ruleExecuterMock, Execute).WillRepeatedly(DoAll(Invoke(PrintExecRule), Return(ERR_OK)));

    DaoFactory::instance_ = std::make_shared<DaoFactory>();
...
HWTEST_F(IptablesSereviceTest, TestRemoveFilewallSuccess, TestSize.Level1)
{
    EXPECT_CALL(*ruleExecuterMock, Execute).WillRepeatedly(DoAll(Invoke(PrintExecRule), Return(ERR_OK)));
    ...
    for (const auto &item : validRules) {
        FirewallRuleParcel validFirewallRule{item};
        ErrCode ret = iptablesService->RemoveFirewallRule(validFirewallRule);
        EXPECT_EQ(ret, ERR_OK);
    }
}

HWTEST_F(IptablesSereviceTest, TestRemoveFilewallFail, TestSize.Level1)
{
    EXPECT_CALL(*ruleExecuterMock, Execute).WillRepeatedly(DoAll(Invoke(PrintExecRule), Return(ERR_OK)));
...
HWTEST_F(IptablesSereviceTest, GetFirewallListTest, TestSize.Level1)
{
    std::string result =
    ...
    ErrCode ret = iptablesService->GetFirewallRules(list);
    ...
    list = {};
    ret = iptablesService->GetFirewallRules(list);
    EXPECT_TRUE(ret == ERR_OK);
    EXPECT_TRUE(list.size() == 1);
}

HWTEST_F(IptablesSereviceTest, TestAddDomainFilterSuccess, TestSize.Level1)
{
    EXPECT_CALL(*ruleExecuterMock, Execute).WillRepeatedly(DoAll(Invoke(PrintExecRule), Return(ERR_OK)));
    ...
    for (const auto &item : validRules) {
        ...
        ErrCode ret = iptablesService->AddDomainFilterRule(validDomainFilterRuleParcel);
        EXPECT_EQ(ret, ERR_OK);
    }
}

HWTEST_F(IptablesSereviceTest, TestAddDomainFilterFail, TestSize.Level1)
{
    EXPECT_CALL(*ruleExecuterMock, Execute).WillRepeatedly(DoAll(Invoke(PrintExecRule), Return(ERR_OK)));
    ...
    for (const auto &item : invalidRules) {
        ...
        ErrCode ret = iptablesService->AddDomainFilterRule(invalidDomainFilterRuleParcel);
        EXPECT_EQ(ret, EdmReturnErrCode::PARAM_ERROR);
    }
}

HWTEST_F(IptablesSereviceTest, TestAddDomainFilterError, TestSize.Level1)
{
    EXPECT_CALL(*ruleExecuterMock, Execute).WillRepeatedly(DoAll(Invoke(PrintExecRule), Return(ERR_OK)));
...
HWTEST_F(IptablesSereviceTest, TestRemoveDomainFilterSuccess, TestSize.Level1)
{
    EXPECT_CALL(*ruleExecuterMock, Execute).WillRepeatedly(DoAll(Invoke(PrintExecRule), Return(ERR_OK)));
    ...
    for (const auto &item : validRules) {
        DomainFilterRuleParcel validDomainFilterRuleParcel{item};
        ErrCode ret = iptablesService->RemoveDomainFilterRules(validDomainFilterRuleParcel);
        EXPECT_EQ(ret, ERR_OK);
    }
}

HWTEST_F(IptablesSereviceTest, TestRemoveDomainFilterFail, TestSize.Level1)
{
    EXPECT_CALL(*ruleExecuterMock, Execute).WillRepeatedly(DoAll(Invoke(PrintExecRule), Return(ERR_OK)));
    ...
    for (const auto &item : invalidRules) {
        DomainFilterRuleParcel invalidDomainFilterRuleParcel{item};
        ErrCode ret = iptablesService->RemoveDomainFilterRules(invalidDomainFilterRuleParcel);
        EXPECT_EQ(ret, EdmReturnErrCode::PARAM_ERROR);
    }
}

HWTEST_F(IptablesSereviceTest, TestGetDomainFilterRules, TestSize.Level1)
{
    std::string result =
    ...
    ErrCode ret = iptablesService->GetDomainFilterRules(list);
    EXPECT_EQ(ret, ERR_OK);
    EXPECT_TRUE(list.size() == 1);
}

HWTEST_F(IptablesSereviceTest, TestGetRemoveChainNameSuccess, TestSize.Level1)
{
    ...
    chainNameList = {};
    ret = iptablesService->GetRemoveChainName(Direction::INPUT, Action::ALLOW, chainNameList);
    EXPECT_TRUE(ret == ERR_OK);
    ...
    chainNameList = {};
    ret = iptablesService->GetRemoveChainName(Direction::INPUT, Action::DENY, chainNameList);
    EXPECT_TRUE(ret == ERR_OK);
    EXPECT_TRUE(chainNameList.size() == 1);
    ...
    chainNameList = {};
    ret = iptablesService->GetRemoveChainName(Direction::INPUT, Action::INVALID, chainNameList);
    EXPECT_TRUE(ret == ERR_OK);
...
HWTEST_F(IptablesSereviceTest, TestGetRemoveChainNameSuccess1, TestSize.Level1)
{
    std::vector<std::string> chainNameList;
    std::vector<std::string> expectList;
    ErrCode ret = ERR_OK;
    ...
    chainNameList = {};
    ret = iptablesService->GetRemoveChainName(Direction::OUTPUT, Action::ALLOW, chainNameList);
    EXPECT_EQ(ret, ERR_OK);
    EXPECT_TRUE(chainNameList.size() == 1);
    ...
    chainNameList = {};
    ret = iptablesService->GetRemoveChainName(Direction::OUTPUT, Action::DENY, chainNameList);
    EXPECT_EQ(ret, ERR_OK);
    EXPECT_TRUE(chainNameList.size() == 1);
    ...
    chainNameList = {};
    ret = iptablesService->GetRemoveChainName(Direction::OUTPUT, Action::INVALID, chainNameList);
    EXPECT_EQ(ret, ERR_OK);
    EXPECT_TRUE(chainNameList.size() == 2);
...
HWTEST_F(IptablesSereviceTest, TestGetRemoveChainNameSuccess2, TestSize.Level1)
{
    std::vector<std::string> chainNameList;
    std::vector<std::string> expectList;
    ErrCode ret = ERR_OK;
    ...
    chainNameList = {};
    ret = iptablesService->GetRemoveChainName(Direction::INVALID, Action::INVALID, chainNameList);
    EXPECT_EQ(ret, ERR_OK);
    EXPECT_TRUE(chainNameList.size() == 4);
}

HWTEST_F(IptablesSereviceTest, TestGetRemoveChainNameFail, TestSize.Level1)
{
    std::vector<std::string> chainNameList;
    std::vector<std::string> expectList;
    ErrCode ret = ERR_OK;
    ...
    chainNameList = {};
    ret = iptablesService->GetRemoveChainName(Direction::INVALID, Action::DENY, chainNameList);
    EXPECT_EQ(ret, EdmReturnErrCode::PARAM_ERROR);

    ret = iptablesService->GetRemoveChainName(Direction::INVALID, Action::ALLOW, chainNameList);
    EXPECT_EQ(ret, EdmReturnErrCode::PARAM_ERROR);
}

HWTEST_F(IptablesSereviceTest, TestExistDenyFirewallRule, TestSize.Level1)
{
    std::string result =
        "Chain edm_deny_output (1 references)\n"
        "num   pkts bytes target     prot opt in     out     source               destination\n"
    ...
    EXPECT_TRUE(iptablesService->ExistDenyFirewallRule());

    EXPECT_CALL(*ruleExecuterMock, Execute).Times(2).WillOnce(DoAll(Return(ERR_OK))).WillOnce(DoAll(Return(ERR_OK)));
    EXPECT_FALSE(iptablesService->ExistDenyFirewallRule());
}

HWTEST_F(IptablesSereviceTest, TestExistDenyDomainRule, TestSize.Level1)
{
    std::string result =
        "Chain edm_dns_deny_output (1 references)\n"
        "num   pkts bytes target     prot opt in     out     source               destination\n"
    ...
    EXPECT_TRUE(iptablesService->ExistDenyDomainRule());

    EXPECT_CALL(*ruleExecuterMock, Execute).Times(1).WillOnce(DoAll(Return(ERR_OK)));
    EXPECT_FALSE(iptablesService->ExistDenyDomainRule());
}

HWTEST_F(IptablesSereviceTest, TestChainExistRule, TestSize.Level1)
{
    std::string result =
        "Chain edm_deny_output (1 references)\n"
        "num   pkts bytes target     prot opt in     out     source               destination\n"
    ...
    EXPECT_TRUE(iptablesService->ChainExistRule(chainNameList));

    EXPECT_CALL(*ruleExecuterMock, Execute).WillRepeatedly(DoAll(Return(ERR_OK)));
    EXPECT_FALSE(iptablesService->ChainExistRule(chainNameList));
}

HWTEST_F(IptablesSereviceTest, TestStaticFunc, TestSize.Level1)
{
    EXPECT_CALL(*ruleExecuterMock, Execute).WillRepeatedly(DoAll(Return(ERR_OK)));

    IptablesService::g_defaultFirewallChainInit = false;
    IptablesService::SetDefaultFirewallDenyChain();
    EXPECT_TRUE(IptablesService::g_defaultFirewallChainInit);

    IptablesService::ClearDefaultFirewallDenyChain();
    EXPECT_FALSE(IptablesService::g_defaultFirewallChainInit);

    IptablesService::g_defaultDomainChainInit = false;
    IptablesService::SetDefaultDomainDenyChain();
    EXPECT_TRUE(IptablesService::g_defaultDomainChainInit);

    IptablesService::ClearDefaultDomainDenyChain();
    EXPECT_FALSE(IptablesService::g_defaultDomainChainInit);
}
} // namespace TEST
} // namespace IPTABLES
} // namespace EDM
