...
#include "enterprise_device_mgr_ability_part_one_fuzzer.h"
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    std::string enterpriseName = CommonFuzzer::GetString(data, pos, stringSize, size);
    std::string description = CommonFuzzer::GetString(data, pos, stringSize, size);
    EntInfo entInfo;
    entInfo.enterpriseName = enterpriseName;
    entInfo.description = description;
