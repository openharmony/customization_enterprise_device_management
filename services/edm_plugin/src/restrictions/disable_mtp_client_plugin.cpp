...
namespace OHOS {
namespace EDM {
const bool REGISTER_RESULT = IPluginManager::GetInstance()->AddPlugin(std::make_shared<DisableMtpClientPlugin>());

DisableMtpClientPlugin::DisableMtpClientPlugin()
{
    EDMLOGI("DisableMtpClientPlugin InitPlugin...");
    policyCode_ = EdmInterfaceCode::DISABLE_MTP_CLIENT;
    policyName_ = PolicyName::POLICY_DISABLED_MTP_CLIENT;
...
ErrCode DisableMtpClientPlugin::CheckConflictPolicy(int32_t userId)
{
    ...
    for (size_t i = 0; i < accounts.size(); i++) {
        ...
        if (disableUserMtpClientPolicy == "true") { // 如果用户级存在只读策略时，设备级想设置为禁用策略，则返回策略冲突
