...
const std::string SETTINGS_DATA_BASE_URI = "datashare:///com.ohos.settingsdata/entry/settingsdata/USER_SETTINGSDATA_";
    ...
    int32_t userId)
{
    ...
    if (!data.empty()) {
        ...
        ErrCode code = EdmDataAbilityUtils::UpdateSettingsData(uri, KEY_USER_DEFINED_DEVICE_NAME, data);
        code = EdmDataAbilityUtils::UpdateSettingsData(uri, KEY_DISPLAY_DEVICE_NAME, data);
        if (FAILED(code)) {
            EDMLOGE("SetDeviceNamePlugin::set deviceName failed : %{public}d.", code);
    ...
    int32_t userId)
{
    ...
    std::string result;
    std::string uri = SETTINGS_DATA_BASE_URI + std::to_string(userId) + SETTINGS_DATA_PREFIX;
    ErrCode code = EdmDataAbilityUtils::GetStringFromSettingsDataShare(uri, KEY_USER_DEFINED_DEVICE_NAME, result);
    if (code != ERR_OK) {
