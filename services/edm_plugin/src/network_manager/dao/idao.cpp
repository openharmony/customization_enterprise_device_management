...
IDao::IDao(std::string chainName) : chainName_(std::move(chainName))
{
    tableName_ = "filter";
}

ErrCode IDao::CreateChain()
{
    std::ostringstream oss;
    oss << SELECT_TABLE_OPTION << tableName_ << NEW_OPTION << chainName_;
    std::string result;
    return RuleExecuter::GetInstance()->Execute(oss.str(), result);
}

ErrCode IDao::Add(const std::shared_ptr<ChainRule>& rule)
{
    if (rule == nullptr) {
...
ErrCode IDao::Remove(const std::shared_ptr<ChainRule>& rule)
{
    std::ostringstream oss;
    oss << SELECT_TABLE_OPTION << tableName_;
...
ErrCode IDao::ExecWithOption(std::ostringstream& oss, const std::shared_ptr<ChainRule>& rule)
{
    oss << rule->Parameter();
    ...
    ErrCode ret = RuleExecuter::GetInstance()->Execute(logRule, result);
    if (ret != ERR_OK) {
        EDMLOGE("ExecWithOption: exec interception rule log fail.");
    }
    return RuleExecuter::GetInstance()->Execute(interceptRule, result);
}

ErrCode IDao::GetAll(std::vector<std::string>& ruleList)
{
    std::ostringstream oss;
    oss << SELECT_TABLE_OPTION << tableName_ << " -n -v -L " << chainName_ << " --line-number";
    std::string result;
    ErrCode ret = RuleExecuter::GetInstance()->Execute(oss.str(), result);
    if (ret != ERR_OK) {
        return ret;
    }
