...
FirewallChainRule::FirewallChainRule(FirewallRule firewallTuple) :
    ChainRule(),
    srcPort_(std::get<5>(firewallTuple)),
    destPort_(std::get<6>(firewallTuple)),
    appUid_(std::get<7>(firewallTuple))
{
    target_ = RuleUtils::EnumToString(std::get<1>(firewallTuple));
    prot_ = RuleUtils::EnumToString(std::get<2>(firewallTuple));
    srcAddr_ = std::get<3>(firewallTuple);
    destAddr_ = std::get<4>(firewallTuple);
}
