...
#ifndef SERVICES_EDM_PLUGIN_INCLUDE_SECURITY_MANAGER_USER_EXT_SESSION_PLUGIN_H
#define SERVICES_EDM_PLUGIN_INCLUDE_SECURITY_MANAGER_USER_EXT_SESSION_PLUGIN_H

#include "iplugin.h"

namespace OHOS {
namespace EDM {
class UserExtSessionPlugin : public IPlugin {
public:
    UserExtSessionPlugin();
    virtual ~UserExtSessionPlugin() = default;
...
protected:
    virtual int32_t CallOpenSession(int32_t accountId, std::vector<uint8_t> &challenge);
    virtual int32_t CallCloseSession(int32_t accountId);
};
} // namespace EDM
} // namespace OHOS
